...
_LLM_PERSONAS = _LLM_PARAMETERS.get(
    "personas",
    [
        {
            "weight": 40,
...
_LLM_SENTENCE_COUNTS = _LLM_PARAMETERS.get(
    "sentences", (1, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 4, 4, 5)
)
_LLM_TOKEN_TARGET = _LLM_PARAMETERS.get(
    "tokenTarget", 7800
)  # should be a little below what the provider supports
...
def _calculate_persona_weighting():
    ...
    for persona in _LLM_PERSONAS:
        ...
        _LLM_PERSONA_WEIGHTING.append(
            (cumulative / total_weight, persona["personality"])
        )
...
_LLM_CHANNEL_BUFFERS = collections.defaultdict(
    lambda: collections.deque(maxlen=_LLM_BUFFER_SIZE)
)  # entries are ("user"|"assistant", message)
...
_ChannelContext = collections.namedtuple(
    "ChannelContext", ["id", "responding", "learning"]
)
    ...
    if message.channel.type == discord.ChannelType.text:
        if message.channel.id in CHANNEL_IDS_TO_CONTEXTS:
            ...
            if _query_permission(message.guild.id, message.author.id):
                summary.append(
                    "Your text will be used to teach the chatbot; `!tyuo disable` to stop."
                )
            else:
                summary.append(
                    "Your text will not be used to teach the chatbot; `!tyuo enable` to opt in."
                )
            summary.append(
                "`!tyuo status` can also be used to see whether you've opted in as a teacher on this server."
            )
...
{tyuo_content}""",
    ...
    async with httpx.AsyncClient() as client:
        ...
        for thinking_token in ("</think>", "</thinking>", "</anth thinking>"):
            if thinking_token in output:
                output = output.rsplit(thinking_token, 1)[1].strip()
        ...
        if message.content.startswith("!tyuo "):
            ...
            if content == "status":
                if _query_permission(guild_id, user_id):
                    await message.reply(
                        "You are currently teaching the chatbot on this server. Thank you!\nIf you want to stop, use `!tyuo disable`.",
                        mention_author=False,
                    )
                else:
                    await message.reply(
                        "You are not currently teaching the chatbot on this server.\nIf you want to start, use `!tyuo enable`.",
                        mention_author=False,
                    )
            elif content == "enable":
                _grant_permission(guild_id, user_id)
                await message.reply(
                    "Thank you for opting in to teaching the chatbot; use `!tyuo status` if you forget that you did this.",
                    mention_author=False,
                )
            elif content == "disable":
                _revoke_permission(guild_id, user_id)
                await message.reply(
                    "Done. You're no longer teaching the chatbot; use `!tyuo status` if you forget that you did this.",
                    mention_author=False,
                )
            ...
            if (
                not c
            ):  # don't respond to empty pings, since these are intended to trigger help
                return False
            ...
            try:
                ...
                await message.reply(
                    "Something went wrong. The chatbot might be down.",
                    mention_author=False,
                )
                raise
            else:
                if results:
                    ...
                    for result in results:
                        results_by_score[math.floor(result["Score"])].append(
                            (result["Utterance"])
                        )
                    ...
                    if llm_process:
                        ...
                        for i in range(3):
                            try:
                                async with message.channel.typing():
                                    (utterance, persona) = await _llm_augment(
                                        tyuo_utterance,
                                        _gather_context(message.channel.id),
                                    )
                                    _record_context(
                                        message.channel.id, "assistant", utterance
                                    )
                                    if peek_llm_inputs:
                                        utterance += (
                            ...
                            _record_context(
                                message.channel.id, "assistant", tyuo_utterance
                            )
                            await message.reply(
                                f"Something went wrong with the LLM layer.\n`tyuo` response: `{tyuo_utterance}`",
                                mention_author=False,
                            )
                    ...
                    if debug_display:
                        fake_selection_pool = selection_pool[
                            :10
                        ]  # limit size to make Discord happy
                        if tyuo_utterance not in fake_selection_pool:
                            fake_selection_pool[
                                random.randint(0, len(fake_selection_pool) - 1)
                            ] = tyuo_utterance
                    ...
                    try:
                        ...
                        await message.reply(
                            f"Something went wrong when sending the response to Discord: {e}",
                            mention_author=False,
                        )
                else:
                    if _query_permission(guild_id, user_id):
                        await message.reply(
                            "I don't know enough to respond; please converse in my presence so I can learn more.",
                            mention_author=False,
                        )
                    else:
                        await message.reply(
                            "I don't know enough to respond; talk to others around me so I can learn.",
                            mention_author=False,
                        )
